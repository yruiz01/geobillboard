...
class Alquiler extends Controller
{
    ...
    public function registrar()
    {
        ...
        if (
            ...
            if (empty($existe)) {
                ...
                $msg = array('msg' => 'La valla esta reservada: ' . $existe['f_recogida'] . ' hasta ' . $existe['f_entrega'], 'icono' => 'error');
            }
        }
    ...
    public function pdfPrestamo($id)
    {
        ...
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(135, 10, utf8_decode('Datos de la Valla'), 0, 1, 'C');
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(65, 5, utf8_decode('No. VALLA: ' . $data['placa']), 1, 0, 'L');
        $pdf->Cell(70, 5, utf8_decode('TIPO: ' . $data['tipo']), 1, 1, 'L');
        $pdf->Cell(65, 5, utf8_decode('MATERIAL: ' . $data['modelo']), 1, 0, 'L');
        //COMPROBAR TIPO
    ...
    public function pdfAlquiler()
    {
        ...
        if (empty($alquiler)) {
            ...
            $pdf->Cell(30, 5, 'No. Valla', 1, 0, 'L', true);
            $pdf->Cell(40, 5, utf8_decode('Tipo de Valla'), 1, 0, 'L', true);
            $pdf->Cell(35, 5, 'F. Inicio', 1, 0, 'L', true);
            $pdf->Cell(35, 5, 'F. Final', 1, 0, 'L', true);
            $pdf->Cell(15, 5, utf8_decode('Cantidad'), 1, 0, 'L', true);
